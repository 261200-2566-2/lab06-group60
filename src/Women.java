...
public class Women implements Character{
    ...
    /**
     * Display the details of the attack between the current character (Women) and another character (Men).
     ...
     *
     */
    public void showDetailAttack(Men men){

        System.out.printf("╔══❖══════════════════⚔\uFE0F══════════════════❖══╗%n");
        System.out.printf("      %2s %2s %2s %n",this.Name,"Attack",men.Name);

        if(damageTaken > 0){
            System.out.printf("%2s %2s %2s %2s%n",this.Name,"Attack more than",men.Name,"Defense");
        }else{
            System.out.printf("%2s %2s %2s %2s%n",this.Name,"Attack lower than",men.Name,"Defense");
        }
    ...
    /**
     * Display the details of the defend between the current character (Women) and another character (Men).
     ...
     *
     */
    public void showDetailDefend(Men men){

        System.out.printf("╔══❖══════════════════⚔\uFE0F══════════════════❖══╗%n");
        System.out.printf("      %2s %2s %2s %n",this.Name,"Defend",men.Name);

        if(damageTaken > 0){
            System.out.printf("%2s %2s %2s %2s%n",this.Name,"Defense more than",men.Name,"Attack");
        }else{
    ...
    /**
     * Perform an defend by taking the parameter as the targeted Men object.
     * @param  men only
     * effects:An defend will cause a change in the HP and bhp value of both the character (this) and the Men.
     * effects:After the Attack is completed, the code will attack the defend details through the method showDetailAttack.
     */

    public void Attack(Men men){

        if (blevel == 0 || bhp == 0 || bmana == 0 || bdamage == 0 || bdefense == 0 || bspeed == 0) {
            this.damageTaken = this.Damage - men.Defense;

            if (this.damageTaken > 0) {
                men.HP -= (int) this.damageTaken;
            } else if (this.damageTaken < 0) {
                this.HP -= (int) Math.abs(this.Damage - men.Defense);
            }
        }

        else {
            this.damageTaken = this.bdamage - men.bdefense;

            if (this.damageTaken > 0) {
                men.bhp -= (int) this.damageTaken;
            } else if (this.damageTaken < 0){
                this.bhp -= (int) Math.abs(this.bdamage - men.bdefense);
            }
        }
        showDetailAttack(men);
     ...
     */
    public void Defend(Men men) {

        if (blevel == 0 || bhp == 0 || bmana == 0 || bdamage == 0 || bdefense == 0 || bspeed == 0) {
            this.damageTaken = this.Defense - men.Damage;

            if (this.damageTaken > 0) {
                men.HP -= (int) damageTaken;
            } else if (this.damageTaken < 0) {
                this.HP -= (int) Math.abs(men.Damage - this.Defense);
            }
        } else {
            this.damageTaken = this.bdefense - men.bdamage;

            if (this.damageTaken > 0) {
                men.bhp -= (int) damageTaken;
            } else if (this.damageTaken < 0) {
                this.bhp -= (int) Math.abs(men.bdamage - this.bdefense);
            }
        }
        showDetailDefend(men);
    }
